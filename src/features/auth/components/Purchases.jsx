...
export const Purchases = () => {
  ...
  const eliminarCompra = (title) => {

    const nuevos = compra.filter((f) => f.title !== title);

    setCompra(nuevos);

    localStorage.setItem("cart", JSON.stringify(nuevos));
...
{compra.length === 0 && (
  <Typography color="white" sx={{ fontFamily: "'Poppins', sans-serif" }}>
    No tienes artículos en Compras
  </Typography>
)}
...
onClick={() => eliminarCompra(articulo.title)}
>
